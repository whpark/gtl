...
namespace gtl {
	...
	#pragma pack(pop)
		...
		if (pixel_size == 3) {
			...
			if (bPixelIndex)
				MatToBitmapFile<true, bLoopUnrolling>(f, img, nBPP, pal);
			else
				MatToBitmapFile<false, bLoopUnrolling>(f, img, nBPP, pal);
	...
	#pragma pack(pop)
		...
		constexpr bool bLoopUnrolling = true;
		if (!MatFromBitmapFile<bLoopUnrolling>(f, img, header.nBPP, palette)) {
			img.release();
		}
		if (bFlipY) {
			cv::flip(img, img, 0);
