...
namespace gtl::win_util {
	...
	bool SaveBitmap(std::filesystem::path const& path, cv::Mat const& img, int nBPP, std::span<RGBQUAD> palette) {
	#pragma pack(push, 2)
	...
	#pragma pack(pop)
		...
		if (pixel_size == 3) {
			...
			fh.offsetData = sizeof(fh) + sizeof(header) + palette.size()*sizeof(palette[0]);
