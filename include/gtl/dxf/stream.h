			...
			if constexpr (eDXFFileType == eDXF_FILE_TYPE::ascii) {
				if constexpr (std::is_arithmetic_v<T>) {
					...
					if constexpr (std::is_same_v<T, bool>)
						value = !!gtl::tszto<int>(str.data(), str.data() + str.size());
					else
						value = gtl::tszto<T>(str.data(), str.data() + str.size());
				}
		...
		binary_t HexStringToBinary(string_t const& str) {
			...
			for (auto sv = gtl::TrimView(std::string_view(str)); !sv.empty(); ) {
				...
				if (sv.size() >= 2 and std::isxdigit(sv[1])) {
					value.push_back(tszto<uint8, char>(sv.data(), sv.data() + 2, nullptr, 16));
					sv = sv.substr(2);
				}
				else {
					value.push_back(tszto<uint8, char>(sv.data(), sv.data() + 1, nullptr, 16));
					sv = sv.substr(1);
				}
			}
			return value;
