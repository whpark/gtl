...
namespace gtl::dxf {
	...
	consteval size_t GetGroupValueIndex() {
		...
		} else if constexpr (std::is_same_v<T, gtl::rad_t> or std::is_same_v<T, gtl::deg_t>) {
	...
	public:
		...
		std::optional<T> GetValue() const {
			...
			if (value.index() != index)
				return std::nullopt;
			if constexpr (std::is_enum_v<T>)
				return static_cast<T>(std::get<index>(value));
			else if constexpr (std::is_same_v<T, gtl::color_bgra_t>) {
				T result{};
				result.cr = static_cast<std::uint32_t>(std::get<index>(value));
				return result;
			}
			else if constexpr (std::is_same_v<T, gtl::rad_t> or std::is_same_v<T, gtl::deg_t>)
		...
		static eGROUP_VALUE_TYPE GetValueTypeEnum(eGROUP_CODE eCode) {
			constinit static auto const s_tblGroupCodeToType = [] {
				...
				for (eGROUP_CODE code{}; code.eCode < tbl.size(); code.eCode++) {
	...
	public:
		...
		constexpr static bool IsFirstGroupCodeMatch(eGROUP_CODE eCode) {
			...
			else if constexpr (CountStructMember<TStruct>() > 1) {
				using member_t = struct_member_t<0, TStruct>;
		...
		bool SetFromGroup(TField& field, group_iter_t& iter) {
			...
			if constexpr (requires (TStruct t) { t.SetFromGroup; }) {
				...
				if (!ReadFieldMembers(value.back(), iter))
					return false;
			}
			return true;
		}
