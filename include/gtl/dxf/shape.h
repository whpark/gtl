...
//////////////////////////////////////////////////////////////////////
//
// shape.h : DXF -> gtl::shape::xDrawing
//
// PWH
// 2026-09-02 ported from biscuit.dxf (module :shape) to gtl.dxf (header)
//
//	NOTE : biscuit::shape keeps UTF-8 (char) strings, gtl::shape keeps wide (wchar_t)
//	       strings. so, unlike biscuit, names / texts must be converted here.
//	       the codepage comes from the DXF header ($ACADVER / $DWGCODEPAGE).
//	       see sShapeConvertContext / CodepageOfDXF.
//
//////////////////////////////////////////////////////////////////////
...
#include "gtl/shape/shape.h"			// -> shape_others.h -> shapes/*.h
...
namespace gtl::dxf::detail {
	...
	inline gtl::eCODEPAGE CodepageOfDXF(std::string_view strACADVER, std::string_view strCodepage) {
	...
	//-----------------------------------------------------------------------------------------------------------------------------
	inline gtl::shape::point_t ShapePoint(point_t const& pt) {
		return gtl::shape::point_t{pt.x, pt.y, pt.z};
	}
	inline gtl::shape::polypoint_t PolyPoint(point_t const& pt, double bulge = {}) {
	...
	inline gtl::shape::color_t IndexedColor(int index, gtl::shape::color_t fallback = gtl::shape::colorTable_s[7]) {
		index = std::abs(index);
	...
	inline gtl::shape::color_t TrueColor(gtl::color_bgra_t color) {
		return ColorRGBA(color);
	}

	/// @brief gtl::shape::xDrawing::Layer() does NOT create a missing layer. (biscuit::shape::xDrawing does)
	inline gtl::shape::xLayer& EnsureLayer(gtl::shape::xDrawing& drawing, layer_map_t& layers, gtl::shape::string_t name) {
		if (name.empty())
			name = L"0"s;
	...
	inline void ApplyCommon(gtl::shape::xShape& target, entities::xEntity const& source, sShapeConvertContext const& ctx) {
	...
	inline std::unique_ptr<gtl::shape::xShape> ConvertSimpleEntity(entities::xEntity const& entity, sShapeConvertContext const& ctx) {
	...
	inline std::unique_ptr<gtl::shape::xPolyline> ConvertPolyline(entities::xPolyline const& source, entities::entities_t const& entities, size_t& index, sShapeConvertContext const& ctx) {
		auto target = std::make_unique<gtl::shape::xPolyline>();
		auto const& field = source.Subclasses().m_field;
	...
	inline gtl::shape::xDrawing ConvertToShape(xSectionTables const& tables, xSectionBlocks const& blocks, xSectionEntities const& sectionEntities,
		gtl::eCODEPAGE eCodepage = gtl::eCODEPAGE::UTF8)
	{
