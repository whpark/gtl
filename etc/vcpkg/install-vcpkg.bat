...
vcpkg install --triplet=x64-windows --recurse catch2 boost flux 7zip bzip2 libarchive fmt imgui spdlog scnlib ctre cpp-peglib ctpg foonathan-lexy ms-gsl magic-enum libenvpp winreg tinyxml2 icu utfcpp libiconv freetype cpr glaze nlohmann-json opengl glew glm glfw3 freeglut tinyspline libxml2 libxmlmm openssl exprtk eigen3 libdmtx freeimage blend2d ffmpeg[all] aravis[introspection,packet-socket,usb] opencv[ade,aravis,calib3d,contrib,core,cuda,cudnn,dnn,dnn-cuda,eigen,ffmpeg,freetype,fs,gapi,gdcm,gstreamer,hdf,highgui,intrinsics,ipp,jpeg,jpegxl,openexr,opengl,openjpeg,openmp,png,python,quality,quirc,rgbd,sfm,text,thread,tiff,webp] vtk[atlmfc,opengl] wxwidgets[debug-support,example,fonts,media,webview] eventpp xlnt sqlitecpp
...
if Exist "%VCPKG_FOLDER%\installed\x64-windows\include\opencv4\opencv2" (
	echo !====
	echo !  junction link include/opencv2 to include/opencv4/opencv2/
	echo !====
	pushd %VCPKG_FOLDER%\installed\x64-windows\include\
	mklink /j opencv2 opencv4\opencv2
	popd
)
